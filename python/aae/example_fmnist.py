...
#    Uniform labeling process
#    idx_unlabel = np.random.randint(0, x_train.shape[0], 50000)
#    y_train[idx_unlabel] = 10
    
#    Non-uniform labeling process
    N_train = np.size(y_train)
#    
    # Unbalanced numbers of labeled data in different classes
#    num_labeled = np.array([1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000])
    num_labeled = np.round(np.random.dirichlet(np.ones(10)/2,size=1)*10000).astype(int)[0]
    ...
    index_unlabeled = np.setdiff1d(np.arange(N_train), index_labeled)
    
#    x_train_labeled = x_train[index_labeled, :, :]
#    y_train_labeled = y_train[index_labeled]
#    x_train_unlabeld = x_train[index_unlabeled, :, :]
#    y_train_unlabeld = y_train[index_unlabeled]
    
    y_train[index_unlabeled] = 10
